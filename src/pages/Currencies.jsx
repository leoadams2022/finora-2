...
export const Currencies = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      showSuccess(
        `Currency ${formData.code.toUpperCase()} saved successfully.`,
      );
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white flex items-center space-x-2 sm:space-x-2.5">
            <Coins className="h-5 w-5 sm:h-6 sm:w-6 text-emerald-600 shrink-0" />
            <span>Currencies & Exchange Rates</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5 sm:mt-1">
            Manage supported currencies and set exchange rates relative to 1
            USD. Finora automatically triangulates all cross-currency
            conversions.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {currencies.map((curr) => (
          <div
            key={curr.code}
            className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3.5 sm:p-4 shadow-sm flex flex-col justify-between space-y-3 sm:space-y-4 hover:shadow-md transition"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
                <span className="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-700 font-bold text-slate-800 dark:text-slate-200 text-sm sm:text-base">
                ...
                <div className="min-w-0">
                  <div className="flex items-center space-x-1.5 sm:space-x-2 flex-wrap gap-y-1">
                    ...
                    {curr.isBase && (
                      <span className="rounded bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 px-1.5 sm:px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider shrink-0">
                        Base
                      </span>
                    )}
                    {curr.isDefault && (
                      <span className="inline-flex items-center space-x-1 rounded bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 px-1.5 sm:px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider shrink-0">
            ...
            <div className="rounded-lg bg-slate-50 dark:bg-slate-900/50 p-2.5 sm:p-3 text-xs flex items-center justify-between border border-slate-100 dark:border-slate-700/60 gap-2">
              <span className="text-slate-500 font-medium flex items-center space-x-1.5 shrink-0">
                <ArrowRightLeft className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-t-2xl sm:rounded-xl bg-white p-4 sm:p-6 shadow-xl dark:bg-slate-800 border-t sm:border border-slate-200 dark:border-slate-700 max-h-[90vh] overflow-y-auto">
            <h3 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-700 pb-3 truncate">
              {currencyToEdit
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-2.5 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500 uppercase disabled:opacity-50 transition min-h-10.5"
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative flex items-center">
                  <span className="absolute left-3 text-xs text-slate-400 font-medium select-none">
                    1 USD =
                  </span>
                  <input
                    type="number"
                    ...
                    className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 py-2.5 pl-16 pr-12 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500 disabled:opacity-50 font-mono transition min-h-10.5"
                  />
